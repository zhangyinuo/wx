...
include "/home/jingchun.zhang/svn/sys_dev/net_monitor/wx/snoopy.class.php";
class Wechatext
{
	private $cookie;
	private $_cookiename;
	...
	private $_account;
	private $_password;
	private $_datapath = '/home/jingchun.zhang/svn/sys_dev/net_monitor/wx_dev/data/cookie_';
	private $debug;
	...
	public function send_type($id,$content, $type)
	{
		$send_snoopy = new Snoopy; 
		$post = array();
		$post['tofakeid'] = $id;
	...
	public function send_pic_text($id,$content)
	{
		$send_snoopy = new Snoopy; 
		$post = array();
		$post['tofakeid'] = $id;
        ...
        $send_snoopy->referer = "https://mp.weixin.qq.com/cgi-bin/singlesendpage?t=message/send&action=index&tofakeid={$id}&token={$this->_token}&lang=zh_CN";
		$send_snoopy->rawheaders['Cookie']= $this->cookie;
		$submit = "https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token={$this->_token}&lang=zh_CN";
		$send_snoopy->submit($submit,$post);
		$this->log($send_snoopy->results);
		return $send_snoopy->results;
	}

	public function get_someone_msg($id)
	{
		$send_snoopy = new Snoopy; 
		$post = array();
		$post['tofakeid'] = $id;
		...
		$post['content'] = $content;
		$post['ajax'] = 1;
        $submit = "https://mp.weixin.qq.com/cgi-bin/singlesendpage?t=message/send&action=index&tofakeid={$id}&token={$this->_token}&lang=zh_CN";
		$send_snoopy->rawheaders['Cookie']= $this->cookie;
	...
	public function getAllUser()
	{
		$send_snoopy = new Snoopy; 
		$send_snoopy->rawheaders['Cookie']= $this->cookie;
		$submit = "https://mp.weixin.qq.com/cgi-bin/contactmanagepage?t=wxm-friend&token=".$this->_token."&lang=zh_CN&pagesize=10&pageidx=0&type=0&groupid=0";
		$send_snoopy->referer = $submit;
		$post = array('ajax'=>1,'token'=>$this->_token);
		$send_snoopy->fetch($submit);
		$this->log($send_snoopy->results);
		$result = json_decode($send_snoopy->results,1);
	...
	public function getTopMsg(){
		$send_snoopy = new Snoopy; 
		$send_snoopy->rawheaders['Cookie']= $this->cookie;
//		$send_snoopy->rawheaders['Referer']= "https://mp.weixin.qq.com/";
	...
	public function getMsgByFakeid($fakeid){
		$send_snoopy = new Snoopy; 
		$send_snoopy->rawheaders['Cookie']= $this->cookie;
		$submit = "https://mp.weixin.qq.com/cgi-bin/singlemsgpage?token=$this->_token&fromfakeid=$fakeid&msgid=&source=&count=10&t=wxm-singlechat&lang=zh_CN";
//		$post = array('ajax'=>1,'token'=>$this->_token);
		$send_snoopy->referer = $submit;
		$send_snoopy->submit($submit,$post);
		$pos = strpos($send_snoopy->results,"type=\"json\">");
		if ($pos === false)
		{
			$this->log($send_snoopy->results);
			$this->log($send_snoopy->header);
			$this->log($this->cookie);
			$this->log($submit);
			$this->log("err result!\n");
			return false;
		}
		$msg = substr($send_snoopy->results, $pos + strlen("type=\"json\">"));
		$pos = strpos($msg, "</script><script id");
		$msg1 = substr($msg, 0, $pos);

		$this->log($msg1);
		$result = json_decode($msg1,1);
		if(!$result){
			return false;
		}
		return $result;
	...
	public function login(){
		...
		$post["f"] = "json";
		$post["imgcode"] = "";
		$snoopy->rawheaders['Referer']= "https://mp.weixin.qq.com/";
		$snoopy->rawheaders['Host']= "mp.weixin.qq.com";
		$snoopy->rawheaders['X-Requested-With']= "XMLHttpRequest";
		#$snoopy->rawheaders['Accept']= "application/json, text/javascript, */*; q=0.01";
		$snoopy->submit($submit,$post);
		$cookie = '';
		$headers = print_r($snoopy->headers, true);
		$this->log("head:".$headers);
		$this->log("result".$snoopy->results);
		...
		if ($cookie) {
			$s = strpos($snoopy->results, "token=");
			$mysub = substr($snoopy->results, $s+6, -1);
			$s = strpos($mysub, "\"");
			
			$this->_token = substr($mysub, 0, $s);
	...
	public function checkValid()
	{
		$send_snoopy = new Snoopy; 
		$post = array('ajax'=>1,'token'=>$this->_token);
		$submit = "https://mp.weixin.qq.com/cgi-bin/getregions?id=10165&t=ajax-getregions&lang=zh_CN";
		$send_snoopy->rawheaders['Cookie']= $this->cookie;
		$send_snoopy->submit($submit,$post);
		$result = $send_snoopy->results;
